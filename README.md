...
| **v2.1.0** | Students who want the reference **plus database-backed sessions** | Everything in v2.0.0 plus **Database-Backed Session Store & Active Sessions List**: tracks active sessions in the database mapping session IDs to user IDs, user-agent, IP, and activity times. A pure-ASGI middleware verifies sessions, immediately clearing cookie and redirecting to `/login` when revoked. Shows an 'Active Sessions' card on `/profile` with options to revoke individual sessions or log out all other devices. Sixth DB-schema change (1 table `sessions` created). |
