...
**Live Demo of v0.1.0:** https://vuln-web-app.onrender.com | **EC2 Live Demo:** http://51.21.201.20.sslip.io:3001
...
| **v2.0.1** | Students who want the reference **plus password reset flow** | Everything in v2.0.0 plus **Password Reset Flow**: timed, single-use password recovery flow. Users enter email/username at `POST /forgot-password` to receive a cryptographically secure token link via email. Validated server-side with strict 1-hour expiration and immediately cleared on successful reset to prevent reuse, while resetting lockout state. |
| **v2.0.2** | Students who want the reference **plus secure file uploads** | Everything in v2.0.1 plus **Secure File Uploads (Avatars)**: supports custom profile pictures on `/profile`. Enforces size limits (<2MB), strips user-supplied filenames to close Path Traversal, and checks magic byte headers to prevent SVG/HTML injection (XSS/RCE). |
| **v2.1.0** | Students who want the reference **plus database-backed sessions** | Everything in v2.0.2 plus **Database-Backed Session Store & Active Sessions List**: tracks active sessions in the database mapping session IDs to user IDs, user-agent, IP, and activity times. A pure-ASGI middleware verifies sessions, immediately clearing cookie and redirecting to `/login` when revoked. Shows an 'Active Sessions' card on `/profile` with options to revoke individual sessions or log out all other devices. Sixth DB-schema change (1 table `sessions` created). |
| **v2.2.0** | Students who want the reference **plus admin control panel** | Everything in v2.1.0 plus **Admin Control Panel (RBAC)**: introduces role-based access control. Admin accounts can visit `/admin` to monitor registered users, manage user roles (promote/demote), delete accounts, or revoke all active sessions for any user. Self-demotion and self-deletion are strictly guarded. Seventh DB-schema change (adds `role` column to `users` defaulting to `'user'`). |
| **v2.3.0** | Students who want the reference **plus secure audit logging** | Everything in v2.2.0 plus **Sanitized Security Audit Logging**: logs all authentication, security config, file upload, and administrative events to `security_audit.log`. Implements a custom CRLF injection sanitizer that escapes carriage returns and line feeds (preventing log forging / forgery attacks). |
| **v2.4.0** | Students who want the reference **plus advanced vulnerability labs** | Everything in v2.3.0 plus **Advanced Vulnerability Labs & Progress Tracking**: adds Command Injection, SSRF, and XXE labs with Vulnerable and Secure modes, a database-backed progress tracking table (`lab_progress`), and official `Dockerfile` + `docker-compose.yml` configurations. |

The incremental tags between them (**v0.1.2 – v0.1.7**) each close one additional vulnerability — see the [Bug Fixes](#bug-fixes) table for the version-by-version mapping. The feature-enhancement tags build on top of v1.0.0: **v1.0.1** adds the password strength meter, **v1.0.2** the User Profile Page, **v1.0.3** Continue with Google, **v1.0.4** Email Verification on Signup, **v1.0.5** Account Lockout, **v1.0.6** Email OTP 2FA, **v1.0.7** MFA via Authenticator App (TOTP), **v1.0.8** QR Code Login, **v2.0.0** CAPTCHA on Login, **v2.0.1** Forgot Password Reset, **v2.0.2** Secure File Uploads, **v2.1.0** Database-Backed Sessions, **v2.2.0** Admin Control Panel, **v2.3.0** Secure Audit Logging, and **v2.4.0** Advanced Labs & Progress Tracking.
...
| POST | `/profile/sessions/revoke` | Revoke a specific active database session (returns JSON) | Yes |
| POST | `/profile/sessions/revoke-all` | Revoke all other active database sessions (returns JSON) | Yes |
| POST | `/profile/avatar` | Upload profile picture avatar (returns JSON) | Yes |
| GET | `/forgot-password` | Request password reset page | No |
| POST | `/forgot-password` | Submit password reset request (returns JSON) | No |
| GET | `/reset-password` | Verification and form page to reset password | No |
| POST | `/reset-password` | Submit new password to reset it (returns JSON) | No |
...
| GET | `/admin` | Display Administrator Control Panel | Yes (Admin) |
| POST | `/admin/user/promote` | Promote standard user to Admin or demote Admin to User | Yes (Admin) |
| POST | `/admin/user/delete` | Permanently delete user account and sessions | Yes (Admin) |
| POST | `/admin/user/revoke-sessions` | Revoke all active sessions for a specific user | Yes (Admin) |
...
| **v2.0.1** | 13 | Password Reset Flow | Timed, single-use password recovery flow. Users request reset at `POST /forgot-password` generating a cryptographically secure `secrets.token_urlsafe(32)` token emailed via a link using configured `APP_BASE_URL` (neutralizing Host Header Injection). Verified server-side with a strict 1-hour expiration and cleared on successful reset to prevent reuse, resetting the lockout counter. Sixth DB-schema change (adds `reset_token` and `reset_token_expires` to `users`). | **Done** |
| **v2.0.2** | 14 | Secure File Uploads | Custom avatars on `/profile` via `POST /profile/avatar` (session-gated and CSRF-protected). Prevents unrestricted file uploads by validating extensions, verifying magic byte headers for PNG/JPEG/GIF, restricting file size to <2MB, and saving as `avatar_{user_id}_{token}.{ext}` (preventing Path Traversal). Clears old files on disk. Seventh DB-schema change (adds `picture` to `users`). | **Done** |
| **v2.1.0** | 10 | Database-Backed Sessions | Tracks active user sessions in a SQLite `sessions` table. Implements ASGI `verify_db_session_middleware` validating every authenticated request. Adds an "Active Sessions" card on the Profile page `/profile` displaying clean parsed device information (e.g. "Chrome on Windows"), IP, and last activity timestamps. Supports single session revocation and global bulk revocation of all other active sessions, immediately clearing cookie signatures and forcing a redirect on the next request. Eighth DB-schema change (1 table `sessions` created). | **Done** |
| **v2.2.0** | 11 | Admin Control Panel | Role-based access control (RBAC) dashboard `/admin` for administrators. Allows monitoring of registered user details and active session counts. Enables admins to promote/demote user roles, delete accounts permanently, and revoke all active sessions for any user. Self-demotion and self-deletion are strictly prevented. Ninth DB-schema change (adds `role` column to `users` table). | **Done** |
| **v2.3.0** | 12 | Secure Audit Logging | Centralized rotating security log `security_audit.log` tracking authentication actions, role modifications, session revocations, password resets, and file uploads. Integrates a custom CRLF injection sanitizer that escapes carriage return and line feed characters in all logged parameters, neutralizing log forgery/injection exploits. | **Done** |
| **v2.4.0** | 15 | Advanced Labs & Progress Tracking | Adds three new Advanced Labs (Command Injection, SSRF, and XXE) with Vulnerable and Secure modes to let students practice exploiting and mitigating vulnerabilities. Tracks progress in a new `lab_progress` table and provides `Dockerfile` + `docker-compose.yml` environment configurations. | **Done** |
